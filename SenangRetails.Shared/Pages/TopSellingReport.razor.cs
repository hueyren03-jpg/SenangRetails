...
{
    public partial class TopSellingReport
    {
        ...
        private List<TopSalesItem> items = new();
        private List<TopSalesItem> filteredItems = new();
        ...
        // Filter properties
        private string skuFilter = string.Empty;
        ...
        private void ApplyFilters()
        {
            ...
            {
                filteredItems = new List<TopSalesItem>();
                return;
            }
            ...
            {
                query = query.Where(x => (x.itemName != null && x.itemName.Contains(skuFilter, StringComparison.OrdinalIgnoreCase)) ||
                                         (x.ItemID != null && x.ItemID.Contains(skuFilter, StringComparison.OrdinalIgnoreCase)));
            }
        ...
        {
            return !string.IsNullOrWhiteSpace(skuFilter) ||
        ...
        private async Task LoadDataAsync()
        {
            ...
            try
            {
                ...
                var apiResponse = await ReportService.GetTopSalesItemAsync(
                    startDateTime,
                    endDateTime,
                    inventoryTypeId,
                    recordCount,
        ...
        private async Task ExportToCSV()
        {
            ...
            if (HasActiveFilters())
            {
                ...
                if (!string.IsNullOrWhiteSpace(skuFilter))
                    csv.AppendLine($"\"  - SKU Filter\",\"{EscapeCsvValue(skuFilter)}\",");
                if (!string.IsNullOrWhiteSpace(salesFilter))
            ...
            csv.AppendLine($"\"{LangSvc.GetText("ReportItemName")}\",\"{LangSvc.GetText("ReportSales")} (Qty)\",\"{LangSvc.GetText("ReportSales")} (Amount)\",\"{LangSvc.GetText("ReportRedeemed")} (Qty)\",\"{LangSvc.GetText("ReportRedeemed")} (Amount)\",\"{LangSvc.GetText("ReportTotal")} (Qty)\",\"{LangSvc.GetText("ReportTotal")} (Amount)\"");

            foreach (var item in dataToExport)
            {
                csv.AppendLine($"\"{EscapeCsvValue(item.itemName)}\"," +
            ...
            var fileName = $"TopSelling_{selectedTypeTab}_{startDate:yyyyMMdd}_{endDate:yyyyMMdd}.csv";
        ...
        private async Task ExportToPDF()
        {
            ...
            try
            {
                ...
                var fileName = $"TopSelling_{selectedTypeTab}_{startDate:yyyyMMdd}_{endDate:yyyyMMdd}.pdf";
        ...
        private byte[] GeneratePdfDocument(List<TopSalesItem> dataToExport)
        {
                    ...
                    page.Content()
                        ...
                        .Column(column =>
                        {
                            ...
                            if (HasActiveFilters())
                            {
                                ...
                                if (!string.IsNullOrWhiteSpace(skuFilter))
                                    column.Item().Text($"  - SKU: {skuFilter}");
                                if (!string.IsNullOrWhiteSpace(salesFilter))
                            ...
                            column.Item().Text(text =>
                            {
                                text.Span($"Total Sales: RM {totalAmount:N2} | ");
                            ...
                            column.Item().Row(row =>
                            {
                                row.RelativeItem(2).Padding(4).Background(Colors.Brown.Medium).Text("Item Name / SKU").FontColor(Colors.White).Bold().FontSize(8);
                            ...
                            foreach (var item in dataToExport)
                            {
                                column.Item().Row(row =>
                                {
                                    ...
                                    {
                                        col.Item().Text(item.itemName ?? "").FontSize(8).Bold();
                                        col.Item().Text(item.ItemID ?? "").FontSize(7).FontColor(Colors.Grey.Darken1);
                                    });
