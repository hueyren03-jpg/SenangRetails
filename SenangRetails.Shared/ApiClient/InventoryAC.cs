...
namespace SenangRetails.Shared.ApiClient
{
    public class InventoryAC : BaseAC
    {
        ...
        public async Task<ApiResponseRoot<object>?> UpdateProductFullAsync(InventoryFullRequest request)
        {
            ...
            var inventory = GetProperty(payload, "objInventory")!.AsObject();
            ...
            return await PostRawJsonAsync<ApiResponseRoot<object>>("api/InventoryFull/Update", payload.ToJsonString());
        }

        private async Task<JsonObject?> LoadFullPayloadAsync(string? masterAccountId)
        {
            if (string.IsNullOrWhiteSpace(masterAccountId) || !await SetBearerToken())
                return null;
            ...
            string? brandName = null)
        {
            ...
            return await PostRawJsonAsync<ApiResponseRoot<object>>("api/InventoryFull/Update", node.ToJsonString());
        }
